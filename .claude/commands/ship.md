...
`gh pr view <PR번호> --json reviews`로 최종 APPROVED 확인 → `gh pr merge <PR번호> --auto --squash` (ask) — 집행·직렬화는 **직렬화 장치**가 담당(표준 = auto-merge + 브랜치 보호 "Require branches to be up to date" / EC 조직은 merge queue 대체 가능).

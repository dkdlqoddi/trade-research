---
description: 볼트 출하 9단계 — 최종 push, PR 종합 보고, Ready, G2 승인 후 merge queue, 이슈 출하 코멘트, 정리. 순서가 곧 안전장치.
argument-hint: NNN
---
...
`gh pr checks <PR번호> --watch` — verify·e2e·budgets 전부 green까지.
...
`gh pr view <PR번호> --json reviews`로 최종 APPROVED 확인 → `gh pr merge <PR번호> --auto --squash` (ask).
**G2 승인 이후 어떤 커밋도 push하지 않는다.**

**merge queue 충돌 탈락 = 유일 예외**: `git fetch origin && git merge origin/main` 병합 커밋 **하나만** push(기능 변경 절대 금지) → 승인 휘발 → 리뷰어에게 "충돌 해소만 — 재승인 요청" 코멘트(ask) → 재 `--auto`. diff가 병합 커밋뿐임은 리뷰어 측 /review가 확인한다.

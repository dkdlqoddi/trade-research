...
- 아직 `draft`면: `gh pr view <PR번호> --json reviews`로 **APPROVED 리뷰** 확인. 없으면 중단("G1 미승인 — 리뷰어의 /review 대기"). **솔로 폴백 운용 중엔**(CLAUDE.md 현 운용 상태) 타 팀원 Approve 대신 **운영자 터미널 승인**이 G1 — 터미널 확인(ask) 후 동일 스탬프(`approved_by: "@운영자핸들 (터미널)"`).
...
- 팀 열린 **구현 PR**(tasks.md가 원격에 게시된 PR) ≤ 팀 인원수 + 2. 초과면 중단("리뷰 적체 — /review부터") — 이 중단은 진입 게이트의 기계 판정이지 에스컬레이션 질문이 아니다(화이트리스트 무관). 솔로 운용 = 인원 1 → 한도 3, 동일 적용.

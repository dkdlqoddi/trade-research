...
- (G2) 예측↔실측: 일치|불일치 <근거>
- (G1) 의도·리스크: <5줄 요약 평가, [ASSUMED] 타당성, Closes #N 연결 확인>
**권고: 승인 | 변경 요청** — 근거 3줄
### 변경 요청 체크리스트 (변경 요청일 때)
1. <항목>
2. <항목>
```

**브리프 규약**: 변경 요청은 **리뷰 본문 최상위에 번호 체크리스트로**(인라인 스레드는 보조) — `gh api`가 deny이므로 작성자 측이 `gh pr view --comments`만으로 전량 수거(`/address`)할 수 있어야 한다.
...
```
사람의 명시 결정 없이 제출 금지. 24시간 내 응답이 팀 SLA. 변경 요청 후의 반영은 작성자의 `/address <PR번호>`가 한다.

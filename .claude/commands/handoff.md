...
2. **owner 갱신** — spec.md frontmatter `owner:` 를 인수자로 (Edit, frontmatter owner 필드만 — 승인 스펙 동결에 대한 guard 허용 예외). tasks.md frontmatter `owner:`도 동일 갱신(tasks.md는 guard 동결 대상이 아니므로 예외 불요 — 일반 편집).
3. **커밋·push** (ask):
   ```bash
   git add specs/NNN-*

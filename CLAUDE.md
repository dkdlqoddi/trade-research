...
(1인 폴백 = `mode: solo` — 게이트 차이: G1/G2가 타 팀원 PR Approve 대신 터미널 승인 + required approvals 0)

> **현 운용 상태 `[ASSUMED]`**: 팀원 1인(@dkdlqoddi)·브랜치 보호 미설정 — **솔로 폴백**으로 운용 중(G1/G2 = 운영자 터미널 지시, main 직접 커밋 + changelog 동반은 constitution 예외, 의도는 터미널 `/spec <설명>` 직행 가능 — `issue: null`). 산출물(스펙·design·tasks·verification)은 팀 모드와 동일하게 전부 생성한다. 팀원 합류 + 브랜치 보호 설정 시 team 절차(Issue 폼 접수 포함)로 원복.
...
- `[SEC]` 보안 취약점은 **비공개 신고**(Security Advisories) — 공개 이슈·PR 본문에 상세 서술 금지, 핫픽스 레인(축소 게이트 + 24h 내 테스트·스펙 소급).
...
- **G2 (출하 승인)** = 같은 PR Ready + required checks green + 최종 Approve → `gh pr merge --auto --squash`(ask). **G2 승인 후 커밋 push 금지**(dismiss stale이 승인 휘발). 유일 예외 = merge queue 충돌 탈락: `origin/main` 병합 커밋 하나만 push → 재승인 요청.
...
2. **guard 훅 배선 확장**: 레퍼런스 §15 jsonc는 PreToolUse를 Bash에만 걸지만, §17의 스펙 동결·보호 경로·페이즈 규율은 편집 경로가 본체이므로 `Edit|Write|MultiEdit`에도 guard.sh 배선.
...
14. **브랜치 보호·merge queue 활성화**: 관리자 1회 설정 대기 항목 — 설정 전까지 솔로 폴백 게이트로 운용. (Issue Forms는 `.github/ISSUE_TEMPLATE/` 5종으로 생성 완료 — main 반영 시 자동 활성.)

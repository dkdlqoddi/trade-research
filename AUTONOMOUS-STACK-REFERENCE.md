...
**버전** R4 (팀 협업 프로파일 · 전원 Claude Code) · 2026-06
...
.claude/hooks/                  # 스크립트 6개 + spec-lint-words.txt, 전부 chmod +x ([4])
...
### [4] 훅 — 스크립트 6개. bash, stdin JSON은 jq 파싱. 차단 = exit 2 + stderr 사유(Claude에게 피드백됨)
...
- `ls -R .claude specs docs .github` / settings.json `jq` 파싱(주석 0) / 에이전트 7개 `model:` grep / 훅 6개 실행권한·`bash -n` / 워크플로 YAML 파싱(actionlint 또는 python yaml) / `grep -RnE "<[A-Z_]+>" .claude .github` **0건** / `gh auth status`(미인증도 보고만).
